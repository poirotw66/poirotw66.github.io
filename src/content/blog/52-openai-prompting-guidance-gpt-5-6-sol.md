...
## 3. 全新 API 參數 text.verbosity 與控制 Breity
...
*   **最佳實踐**：在 API 層使用 `text.verbosity` 設定預設長度，並在 Prompt 中指定 task-specific 的資訊結構（如：要求模型先講結論，再列出支持結論的 3 個關鍵證據）。
...
## 6. 先驗證再交付 (Check Work Before Finishing)
