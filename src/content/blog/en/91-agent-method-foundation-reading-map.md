...
description: "One diagram shows how CoT and WebGPT merge into ReAct, walk Gorilla and IPI on the method spine, and match the hub agent-systems path order (ancestors first, leaves second)."
pubDate: 2026-08-27
...
tldr:
  ...
  - "From ReAct the control point splits four ways: how tools are learned, how experience is written across trials, how real-repo success is scored, and how context is paged; Gorilla and IPI are spine nodes on the tool/retrieval line."
  - "The hub Agent path now starts on this method spine, not OSReward; 2025–26 notes are leaves—later leaderboard numbers must not be written back into the 2023/2024 classics."
audience:
  - "Readers who just finished CoT → WebGPT → ReAct → Toolformer → Gorilla → IPI → SWE-bench → Reflexion → MemGPT → Generative Agents and need a spine"
...
subtitle: "Same walk as the hub agent-systems path: finish the ancestors, open leaves only when needed."
kind: guide
showToc: true
---
...
>
> The hub Agent path now starts on this method spine, not OSReward. Do not write later leaderboard numbers back into the 2023/2024 classics. A few-shot ReAct loop is not an Argus runtime; Toolformer’s next-token API is not MidTool; SWE-bench’s 1.96% is a protocol, not a ceiling.
...
3. **Gorilla** and **Indirect Prompt Injection** are spine nodes on the tool/retrieval line: after catalog-scale retrieve-and-call, retrieved or tool-returned text shares the instruction channel with the user prompt. **Generative Agents** is the memory-line spine node, not optional extra credit.
4. The hub [agent-systems path](/en/paper-reading/#reading-paths) now follows the same order as this page: finish the method ancestors, then open 2025–26 leaves. Those later notes are **leaves**; they do not retire the classics. SWE-Bench ProMax’s 41.2% and later Letta product metrics must not be back-filled into 2023/2024 tables.
...
### Path B · Full classic spine: CoT → WebGPT → ReAct → … → Generative Agents

Follow the method-ancestor segment of the hub [agent-systems path](/en/paper-reading/#reading-paths): [CoT](/en/paper-reading/29-chain-of-thought-prompting/) → [WebGPT](/en/paper-reading/30-webgpt-browser-assisted-qa/) → [ReAct](/en/paper-reading/24-react-interleaved-reasoning-acting/) → [Toolformer](/en/paper-reading/25-toolformer-self-supervised-api-calls/) → [Gorilla](/en/paper-reading/35-gorilla-llm-connected-with-massive-apis/) → [Indirect Prompt Injection](/en/paper-reading/42-indirect-prompt-injection/) → [SWE-bench](/en/paper-reading/26-swe-bench-github-issue-evaluation/) → [Reflexion](/en/paper-reading/27-reflexion-verbal-reinforcement/) → [MemGPT](/en/paper-reading/28-memgpt-context-as-memory-paging/) → [Generative Agents](/en/paper-reading/36-generative-agents-interactive-simulacra/). Stop after those ten; open 2025–26 leaves (OSReward, Argus, and the rest) only when you need that control point. The hub agent-systems path now uses the same ancestors-then-leaves walk as this map.

### Path C · Pick the next paper from the job

| Where the work is stuck | Start with this leaf | Control point it inherits |
| --- | --- | --- |
| Teaching tools, or too many schemas | [Gorilla](/en/paper-reading/35-gorilla-llm-connected-with-massive-apis/) (spine) → [MidTool](/en/paper-reading/23-midtool-agentic-tool-use/), [RAG-MCP](/en/paper-reading/04-rag-mcp/) | Toolformer → Gorilla: tool use on the training / retrieve-and-call / routing side |
| Whether retrieved or tool-returned text shares the instruction channel | [Indirect Prompt Injection](/en/paper-reading/42-indirect-prompt-injection/) (security spine) → when needed, [AgentS4D](/en/paper-reading/12-agents4d-runtime-risks/), [Trajectory Sentinel](/en/paper-reading/14-agent-trajectory-sentinel/), [A2E](/en/paper-reading/19-a2e-agent-auditing-engine/) | Gorilla → IPI: control plane for retrieval and tool returns entering the prompt |
...
| Gorilla | How to retrieve and call over a huge API catalog | APIBench + RAT put document retrieval into finetuning and cut hallucination | [Already on this site](/en/paper-reading/35-gorilla-llm-connected-with-massive-apis/) | Spine node; catalog retrieve+call ≠ MCP product contract; also ≠ MidTool |
| Indirect Prompt Injection | Whether retrieved or tool-returned text shares the instruction channel with the user prompt | Untrusted data enters the prompt via retrieval or tool returns; attackers can control the model indirectly | [Already on this site](/en/paper-reading/42-indirect-prompt-injection/) | Security spine; 2023 control-plane evidence, not a Guard product SLA |
...
- **The hub agent-systems path now matches this map.** Walk the method ancestors first (CoT → … → Generative Agents), then the 2025–26 leaves. This page is still the method-foundation spine guide, not a fourth path type.
