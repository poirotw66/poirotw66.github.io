...
showToc: true
---
**Harness Engineering**, as Martin Fowler frames it for coding agents, is the **outer control loop** around the model: **feedforward guides** that steer behavior before the agent acts, and **feedback sensors** that observe results and trigger correction afterward. Trust is not a vibe—it is **designed control**.
...
>
> Meow~ How can we trust the code written by AI? Trust is not based on feelings, but on designing "sensors" and "control loops" so that they can catch errors before we review them!
>
> **Huahua's engineering note**
>
> Building trust in the Coding Agent relies on designable feedforward and feedback mechanisms (guides/sensors). Implementing control logic in the outer harness can effectively improve first-time accuracy and allow the system to self-correct before manual review.
...
Harness attempts to make experience **explicit**, but Fowler clearly states: the goal is not to eliminate human input, but to direct input toward **intent, trade-offs, and exceptions**—consistent with the distinction between Harness vs Prompt / Context in [Reading Map 13](/en/blog/13-harness-engineering-reading-map/).
...
- **OpenAI Team**: Layered architecture + custom linters/structural tests + periodic GC for drift scanning; concluding that the difficulties lie in "environment, feedback loops, control systems"—echoing [blog 11](/en/blog/11-harness-engineering/).
...
- [Harness Engineering Reading Map](/en/blog/13-harness-engineering-reading-map/)
- [LangChain Harness Anatomy](/en/blog/15-langchain-agent-harness-anatomy/) · [Hashimoto's Six Phases](/en/blog/16-mitchell-hashimoto-harness-origin/)
