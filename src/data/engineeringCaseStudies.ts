...
> = {
  zh: {
    'agentic-rag': {
      ...
      evaluationMethod: '評測分為兩階段獨立驗證：(1) 品質收斂以 v22 固定 100 題 benchmark 進行人工與規則混合評分，逐題記錄完全正確、部分正確、錯誤、例外與邊界；(2) 查詢延遲由後續 rule-first direct workflow 獨立量測，驗證 deterministic 快速路徑對延遲的改善。兩階段口徑分開記錄，不混為單次實測。',
      metrics: [
        { value: '98.0%', label: '加權準確率', note: 'v22 固定 100 題 benchmark，作者評測' },
        { value: '96 / 100', label: '完全正確', note: '其餘 4 題為部分正確，0 題錯誤或不安全' },
        { value: '2.606s', label: '平均延遲', note: '後續 rule-first 評測，相較 v23 減少 1.024s' },
        { value: '0', label: '例外／逾時', note: '100 題 direct workflow 執行結果' },
      ],
      failures: [
        { symptom: 'FastAPI / Swagger UI 預設 filter placeholder（如 additionalProp1）傳入後，BM25 關鍵字搜尋被整批過濾，混合檢索退化為純向量檢索。', correction: '在 filter_metadata 前置處理中加入正規化與清洗函式，自動剔除 placeholder 與無效 key，再送入 BM25 檢索。', lesson: '跨元件介面的預設值是隱形 bug 的主要來源；檢索管線不能假設上游傳入的 filter 總是乾淨的。' },
        { symptom: '早期查詢前處理對每題都呼叫 LLM 分析意圖，流程正確但平均延遲偏高（v23 baseline 達 3.63s）。', correction: '建立 rule-first、LLM-fallback 架構，高信心常見 FAQ 走正規化快速路徑，僅模糊查詢進入 LLM。', lesson: 'Agentic 的價值在於受控決策，不是在每個節點都堆疊模型呼叫；可規則化的邏輯留在規則層。' },
        { symptom: '檢索有結果卻仍回答錯誤或混答，問題集中在跨主題或相似系統的 context 汙染。', correction: '加入文件評分（grade_documents）、focus context 提取、cross-topic trimming 與生成後 evaluator gate。', lesson: 'RAG 的品質瓶頸常在於送入模型的 context 品質，而非生成模型本身的推理能力。' },
      ],
      links: [
      ...
      disclosure: '核心實作程式碼庫目前為作者私人維護；本頁公開架構分層、評測方法、關鍵程式碼結構與可重現的設計取捨。',
    },
    'ocr-automation': {
      ...
      evaluationMethod: '使用多家醫院真實版型進行端對端驗收，逐欄核對健保別、就診日期、醫院科別、總金額與費用明細是否能穩定映射到統一 schema；低解析度與特殊折痕版型保留人工覆核邊界。',
      metrics: [
        { value: '5+', label: '醫院格式正規化', note: '台大、長庚、彰基、榮總、奇美等' },
        { value: '6', label: '處理階段', note: '從影像校正到 JSON contract' },
        { value: '1', label: '統一 API Schema', note: '供下游財務與醫療 API 直接使用' },
      ],
      failures: [
      ...
      disclosure: 'GitHub 公開核心辨識與正規化管線程式庫（poirotw66/ocr_api）；醫療範例僅展示欄位結構與去識別範例，不包含真實病患隱私個資。',
    },
    'agentic-ai-platform': {
      ...
      evaluationMethod: '以能力矩陣逐項驗證路由是否進入正確子流程，並檢查每個模組在成功、空結果與外部 API 失敗時，都能回到符合 LINE 限制的統一回覆契約。',
      metrics: [
        { value: '1', label: '主要入口主流程', note: '集中處理 webhook、路由與回覆' },
        { value: '19', label: '模組化子流程', note: '獨立開發、替換與維護' },
        { value: '5+', label: '能力類別', note: '涵蓋 RAG、查證、新聞、圖像、網頁等' },
      ],
      failures: [
      ...
      disclosure: '工作流與架構文件已在 GitHub 公開（poirotw66/n8n_workflow）；本專案展示模組化架構與分流能力，正式企業級環境需另行補足權限控管、密鑰管理、稽核軌跡與 SLO 監控。',
    },
  },
  en: {
    'agentic-rag': {
      ...
      evaluationMethod: 'Evaluation is conducted in two separate phases: (1) Quality convergence benchmarked on v22 fixed 100-query set with hybrid human and rule scoring, tracking fully correct, partial correct, and safety boundaries; (2) Query latency evaluated in a subsequent rule-first direct workflow run to verify the latency reduction of deterministic fast-paths. Environments and criteria are recorded separately.',
      metrics: [
        { value: '98.0%', label: 'Weighted accuracy', note: 'v22 fixed 100-query benchmark, author-reported' },
        { value: '96 / 100', label: 'Fully correct', note: 'Remaining 4 partial; 0 errors or unsafe responses' },
        { value: '2.606s', label: 'Average latency', note: 'Subsequent rule-first run; -1.024s vs v23' },
        { value: '0', label: 'Exceptions / timeouts', note: '100-query direct workflow execution' },
      ],
      failures: [
        { symptom: 'Default FastAPI/Swagger filter placeholders (e.g. additionalProp1) caused BM25 keyword search to filter out all candidates, degrading hybrid retrieval to vector-only.', correction: 'Added filter normalization and sanitization before BM25 retrieval to strip placeholders and invalid keys.', lesson: 'Default values across component boundaries are silent failure points; retrieval pipelines must sanitize upstream filters.' },
        { symptom: 'Early versions invoked an LLM for intent analysis on every query, yielding correct flows but high average latency (3.63s in v23 baseline).', correction: 'Introduced rule-first routing with LLM fallback, allowing high-confidence FAQs to bypass the LLM via normalized fast paths.', lesson: 'Agentic workflows require controlled decisions, not LLMs at every node; deterministic logic belongs in the rule tier.' },
        { symptom: 'Queries retrieved valid documents but still generated inaccurate answers due to cross-topic context pollution.', correction: 'Implemented document grading, focused context extraction, cross-topic trimming, and post-generation evaluator gates.', lesson: 'RAG failures often stem from noisy context selection rather than generator capability.' },
      ],
      links: [
      ...
      disclosure: 'The core implementation repository is currently private; this page publishes the layered architecture, evaluation methodology, key code patterns, and reproducible trade-offs.',
    },
    'ocr-automation': {
      eyebrow: 'ENGINEERING CASE STUDY · DOCUMENT INTELLIGENCE',
      ...
      evaluationMethod: 'End-to-end acceptance uses real layouts from multiple hospitals. Verification covers insurance type, dates, hospital, department, totals, and line items mapped into one schema, retaining manual review for low-resolution and atypical scans.',
      metrics: [
        { value: '5+', label: 'Hospital formats normalized', note: 'NTU, Chang Gung, CCH, VGHTPE, Chi Mei' },
        { value: '6', label: 'Processing stages', note: 'From image input to JSON contract' },
        { value: '1', label: 'Unified API Schema', note: 'Directly consumable by downstream APIs' },
      ],
      failures: [
      ...
      disclosure: 'The repository publishes the core recognition and normalization flow (poirotw66/ocr_api). Medical samples demonstrate field structure with de-identified data and are not intended for clinical use.',
    },
    'agentic-ai-platform': {
      eyebrow: 'ENGINEERING CASE STUDY · AGENT PLATFORM',
      ...
      metrics: [
        { value: '1', label: 'Primary entry flow', note: 'Owns webhook, routing, and response segmentation' },
        { value: '19', label: 'Modular subflows', note: 'Separately maintainable and replaceable' },
      ...
      disclosure: 'The repository publishes importable workflows (poirotw66/n8n_workflow). Production enterprise environments still require dedicated authorization, secret management, audit, and SLO controls.',
    },
  },
};
