...
description: "用一張圖說明 CoT、WebGPT 如何匯成 ReAct，經 Gorilla、IPI 走方法脊椎，並與論文庫 agent-systems 路徑同序（祖先先、葉子後）。"
pubDate: 2026-08-27
updatedDate: 2026-08-28
tldr:
  - "CoT 只推理不行動，WebGPT 只行動少推理；ReAct 把 thought–action–observation 縫成同一條軌跡。"
  - "從 ReAct 分出四個控制點：工具怎麼學會、經驗怎麼跨 trial 寫進記憶、真實倉庫怎麼計分、context 怎麼分頁；Gorilla 與 IPI 是工具／檢索線上的脊椎節點。"
  - "論文庫 Agent 系統路徑現在從這條方法脊椎起走，不是從 OSReward 起跳；2025–26 葉子是延伸筆記，不要把後來排行榜分數寫回 2023／2024 經典。"
audience:
  - "剛讀完 CoT → WebGPT → ReAct → Toolformer → Gorilla → IPI → SWE-bench → Reflexion → MemGPT → Generative Agents，需要一張脊椎圖的讀者"
...
subtitle: "方法脊椎與論文庫 agent-systems 路徑同序：祖先先讀完，葉子按需再開。"
kind: guide
showToc: true
---
...
> **花花的工程提醒**
>
> 論文庫 Agent 系統路徑現在從這條方法脊椎起走，不是從 OSReward 起跳。不要把後來的排行榜分數寫回 2023／2024 經典。ReAct 的 few-shot 迴圈不是 Argus runtime；Toolformer 的 next-token API 不是 MidTool；SWE-bench 的 1.96% 是協議，不是天花板。
...
3. **Gorilla** 與 **Indirect Prompt Injection** 是工具／檢索線上的脊椎節點：目錄級檢索＋呼叫之後，retrieved／tool 回傳與 user prompt 共用指令通道。**Generative Agents** 是記憶線脊椎，不是選讀加分。
4. 論文庫 [Agent 系統路徑](/paper-reading/#reading-paths) 現在與本頁同序：先走完方法祖先，再開 2025–26 葉子。站上後續精讀是**葉子**，不是把經典作廢；SWE-Bench ProMax 的 41.2%、後來的 Letta 產品數字，都不得回填進 2023／2024 的表。
...
### 路徑 B · 經典脊椎：CoT → WebGPT → ReAct → … → Generative Agents

依 [論文精讀總覽的 Agent 系統路徑](/paper-reading/#reading-paths) 方法祖先段讀完脊椎：[CoT](/paper-reading/29-chain-of-thought-prompting/) → [WebGPT](/paper-reading/30-webgpt-browser-assisted-qa/) → [ReAct](/paper-reading/24-react-interleaved-reasoning-acting/) → [Toolformer](/paper-reading/25-toolformer-self-supervised-api-calls/) → [Gorilla](/paper-reading/35-gorilla-llm-connected-with-massive-apis/) → [Indirect Prompt Injection](/paper-reading/42-indirect-prompt-injection/) → [SWE-bench](/paper-reading/26-swe-bench-github-issue-evaluation/) → [Reflexion](/paper-reading/27-reflexion-verbal-reinforcement/) → [MemGPT](/paper-reading/28-memgpt-context-as-memory-paging/) → [Generative Agents](/paper-reading/36-generative-agents-interactive-simulacra/)。讀完這十篇就停；2025–26 葉子（OSReward、Argus 等）等你真的需要那個控制點再開。論文庫 agent-systems 路徑現在與本頁同一條祖先→葉子走法。

### 路徑 C · 依工作選下一篇

| 你的工作卡住的點 | 先讀這片葉子 | 它繼承的控制點 |
| --- | --- | --- |
| 工具怎麼教、schema 太多 | [Gorilla](/paper-reading/35-gorilla-llm-connected-with-massive-apis/)（脊椎）→ [MidTool](/paper-reading/23-midtool-agentic-tool-use/)、[RAG-MCP](/paper-reading/04-rag-mcp/) | Toolformer → Gorilla：訓練／檢索＋呼叫／路由側的工具使用 |
| retrieved／tool 回傳是否共用指令通道 | [Indirect Prompt Injection](/paper-reading/42-indirect-prompt-injection/)（安全脊椎）→ 需要時再讀 [AgentS4D](/paper-reading/12-agents4d-runtime-risks/)、[Trajectory Sentinel](/paper-reading/14-agent-trajectory-sentinel/)、[A2E](/paper-reading/19-a2e-agent-auditing-engine/) | Gorilla → IPI：檢索與工具回傳進 prompt 的控制面 |
...
| Gorilla | 巨大 API 目錄上如何檢索並呼叫 | 用 APIBench＋RAT 把文件檢索寫進微調，降低幻覺 | [站上已有精讀](/paper-reading/35-gorilla-llm-connected-with-massive-apis/) | 脊椎節點；目錄級檢索＋呼叫 ≠ MCP 產品契約，也不等於 MidTool |
| Indirect Prompt Injection | retrieved／tool 回傳是否與 user prompt 共用指令通道 | 未受信資料經檢索或工具回傳進 prompt，攻擊者可間接控制模型 | [站上已有精讀](/paper-reading/42-indirect-prompt-injection/) | 安全脊椎；2023 控制面證據，不是 Guard 產品 SLA |
...
| Generative Agents | 多 agent 沙盒裡記憶如何支撐計畫 | 觀察寫入 memory stream、週期反思、檢索後規劃；25 人在 Smallville 互動 | [站上已有精讀](/paper-reading/36-generative-agents-interactive-simulacra/) | 脊椎節點；不是 MemGPT 單 agent 分頁，也不是生產 runtime |
...
- **論文庫 agent-systems 路徑已與本頁對齊。** 先走方法祖先（CoT → … → Generative Agents），再接 2025–26 葉子。本頁仍是方法底座的脊椎導覽，不是第四種 path type。
