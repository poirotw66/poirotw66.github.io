...
> **花花的一句話**：喵！如果不給模型配備各種工具，它就只是一個會說話的大腦。加上沙箱、記憶和指令執行，才能變成真正會工作的小幫手！
>
> **花花的工程提醒**：裸模型並非 Agent，必須由期望行為反推所需 Harness 元件（如沙箱環境、Bash 執行、記憶機制與 Context 管理）。透過產品化這些 primitive，才能將模型能力轉化為可交付的工作引擎。
...
Comparing with [Fowler 14](/blog/14-martin-fowler-harness-engineering-review/): Bash outputs + test runners are the carriers of **computational sensors**.
...
This is on the same "observable behavior" track as [Anthropic's long task article 10](/blog/10-effective-harnesses-for-long-running-agents/) which emphasizes Puppeteer MCP and running the dev server successfully before E2E.
...
When weights cannot be changed at runtime, **context injection** is the only standard channel—this is also why [OpenAI 11](/blog/11-harness-engineering/) makes knowledge into repo artifacts.
...
This complements the compaction discussion in [blog 10](/blog/10-effective-harnesses-for-long-running-agents/): Anthropic says compaction alone isn't enough; LangChain says compaction is a Harness necessity, which also needs to be paired with files, plans, and git.
...
Here we can see the **conceptual alignment** between [blog 10](/blog/10-effective-harnesses-for-long-running-agents/)'s initializer/coding agent, feature list, and LangChain's product primitives: both externalize "handoffs and completion definitions."
